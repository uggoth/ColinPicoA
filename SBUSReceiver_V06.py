...
class SBUSReceiverMC6C(ColObjects.ColObj):
    def __init__(self):
        ...
        self.mids = [1092, 998, 962, 977, 1001, 1100]
        self.mins = [689, 601, 201, 572, 201, 201]
        self.maxs = [1489, 1401, 1801, 1366, 1801, 1801]
        ####################################################################################
        self.interpolate_values = [[0,0,0,0,0,0]]*6
        dead_zone = 50
        jitter_zone = 5
        for i in range(self.no_channels):
            self.interpolate_values[i][0] = 100
            self.interpolate_values[i][1] = self.mins[i] + jitter_zone
            self.interpolate_values[i][2] = self.mids[i] - dead_zone
            self.interpolate_values[i][3] = self.mids[i] + dead_zone
            self.interpolate_values[i][4] = self.maxs[i] - jitter_zone
            self.interpolate_values[i][5] = 2000
        ...
        self.steering_interpolator = ColObjects.Interpolator('Steering Interpolator',
                                    self.interpolate_values[self.steering_index], self.default_results)
        self.throttle_interpolator = ColObjects.Interpolator('Throttle Interpolator',
                                    self.interpolate_values[self.throttle_index], self.inverse_results)
        self.updown_interpolator = ColObjects.Interpolator('Up-Down Interpolator',
                                    self.interpolate_values[self.updown_index], self.default_results)
        self.swing_interpolator = ColObjects.Interpolator('Swing Interpolator',
                                    self.interpolate_values[self.swing_index], self.default_results)
        self.switch_interpolator = ColObjects.Interpolator('Switch Interpolator',
                                    self.interpolate_values[self.switch_index], self.default_results)
        self.knob_interpolator = ColObjects.Interpolator('Knob Interpolator',
                                    self.interpolate_values[self.knob_index], self.default_results)
    ...
    def get_mecanum_mix(self):
        ...
        steering, throttle, updown, crab, switch, knob = inputs
        ...
        lf_level = (fwd_levels[0] + spin_levels[0] + crab_levels[0])
        rf_level = (fwd_levels[1] + spin_levels[1] + crab_levels[1])
        lb_level = (fwd_levels[2] + spin_levels[2] + crab_levels[2])
        rb_level = (fwd_levels[3] + spin_levels[3] + crab_levels[3])
    ...
    def get_tank_mix(self):
        ...
        if None in all_inputs:
            self.nones_count += 1
            return self.old_mix_values
        steering, throttle, updown, crab, switch, knob = all_inputs
        ...
        fwd_abs = self.array_abs(fwd_levels)
        spin_abs = self.array_abs(spin_levels)
        lf_level = (fwd_levels[0] + spin_levels[0])
        rf_level = (fwd_levels[1] + spin_levels[1])
        lb_level = (fwd_levels[2] + spin_levels[2])
        rb_level = (fwd_levels[3] + spin_levels[3])
        output_abs = self.array_abs([lf_level, rf_level, lb_level, rb_level])
        biggest_out = max(output_abs)
        if biggest_out == 0:
            return [0,0,0,0]
